...
exit_notify(void)
{
	if (nnio_util_verbose()) {
		...
		info("nanoclient exiting with %d (%s)\n", err,
		     nn_strerror(err));
	}
}
...
main(int argc, char **argv)
{
	...
	int sock = nnio_socket_open(NN_REQ, options.tx_timeout,
	...
	dbg("preparing to receive result from socket ...\n");

	void *rx_data;
	unsigned int rx_data_len;
	rc = nnio_socket_rx(sock, &rx_data, &rx_data_len);
	if (rc < 0) {
	...
	fprintf(stdout, "%s", (char *)rx_data);
	fflush(stdout);

err_eof:
	nnio_free_data(rx_data);
